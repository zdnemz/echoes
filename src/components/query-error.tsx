...
export function QueryError({
  ...
  return (
    <div role="alert" className={`rounded-lg border border-line bg-paper-raised p-6 text-center ${className}`.trim()}>
      <p className="text-[13px] leading-relaxed text-ember">{message}</p>
      {onRetry && (
        <Button variant="outline" size="sm" className="press mt-3 border-line" onClick={onRetry}>
